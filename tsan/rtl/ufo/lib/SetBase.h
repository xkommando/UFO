 ...
 *
 *
 * A wrapper for Set and Map data structure,
 * can be configured with rb-tree or hashmap (the type parameter "C" in GenMap) (Bowen 2017-10-13)
 *
 */
...
public:
  ...
  SetBase(std::initializer_list <std::pair<KeyType, ValueType>> ini)
      ...
      for (auto iter = ini.begin(); iter != ini.end(); ++iter) { core_.insert(Node(iter->first, iter->second)); }
  }
...
//-----------------------------------------------------------------------------
  ...
  size_t count(const KeyType &k) const noexcept { return core_.find(k) == core_.end() ? 0 : 1; }

  void clear() { core_.clear(); }

  void swap(Self_ &other) noexcept { core_.swap(other.core_); }
//-----------------------------------------------------------------------------
//                  iterator
//-----------------------------------------------------------------------------
public:
  ...
  const_iterator cbegin() const noexcept { return core_.begin(); }

  const_iterator cend() const noexcept { return core_.end(); }

//-----------------------------------------------------------------------------
//                  getter
//-----------------------------------------------------------------------------
public:
  reference at(const KeyType &thekey) noexcept { return core_.at(thekey); }

  const_reference at(const KeyType &thekey) const noexcept { return core_.at(thekey); }

  reference front() noexcept { return core_.front(); }

  const_reference front() const noexcept { return core_.front(); }

  const_reference back() const noexcept { return core_.back(); }

  reference back() noexcept { return core_.back(); }

  iterator find(const KeyType &thekey) noexcept { return core_.find(thekey); }

  const_iterator find(const KeyType &thekey) const noexcept { return core_.find(thekey); }

  iterator lower_bound(const KeyType &thekey) noexcept { return core_.lower_bound(thekey); }

  const_iterator lower_bound(const KeyType &thekey) const noexcept { return core_.lower_bound(thekey); }

  iterator upper_bound(const KeyType &thekey) noexcept { return core_.upper_bound(thekey); }

  const_iterator upper_bound(const KeyType &thekey) const noexcept { return core_.upper_bound(thekey); }

  std::pair <const_iterator, const_iterator>
  equal_range(const KeyType &thekey) const noexcept { return core_.equal_range(thekey); }

  CoreType &data() noexcept { return core_; }

  const CoreType &data() const noexcept { return core_; }
...
//        }
  ...
  bool operator==(const Self_ &other) const noexcept { return core_ == other.core_; }

  bool operator<(const Self_ &other) const noexcept { return core_ < other.core_; }

  size_t count(const KeyType &thek) const noexcept { return core_.find(thek) == core_.end() ? 0 : 1; }
...
//        }
    ...
    InsertionAux &&operator()(std::initializer_list <KeyType> ini) {
        for (auto iter = ini.begin(); iter != ini.end(); ++iter) { host_.exclusiveInsertion(Node(*iter)); }
  ...
  Self_ &operator=(std::initializer_list <KeyType> init) {
      core_.clear();
      for (auto iter = init.begin(); iter != init.end(); ++iter) { core_.exclusiveInsertion(Node(iter->getKey())); }
  }
  ...
  size_t count(const KeyType &thek) const noexcept { return lower_bound(thek) == core_.end() ? 0 : 1; }
...
public:
  ...
  struct InsertionAux {
    ...
    InsertionAux &&operator()(std::initializer_list <KeyType> ini) {
        for (auto iter = ini.begin(); iter != ini.end(); ++iter) { host_.duplicatedInsertion(Node(*iter)); }
  ...
  Self_ &operator=(std::initializer_list <KeyType> init) {
      core_.clear();
      for (auto iter = init.begin(); iter != init.end(); ++iter) { core_.duplicatedInsertion(Node(iter->getKey())); }
      return *this;
  }
