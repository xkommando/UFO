 ...
 * @brief:
 *
 *  basic search and sort function (Bowen 2017-10-13)
 *
 */
...
__NAMESPACE_STARTER___BEGIN__
    ...
    #define __starter_move(obj, TypeName)\
...
#define starter_xchg(jia, yi, TypeName)\
        {\
            TypeName tmp = __starter_move(jia, TypeName);\
            jia = __starter_move(yi, TypeName);\
            yi = __starter_move(tmp, TypeName);\
        }

#define starter_swap(jia, yi, bin,TypeName)\
            bin = __starter_move(jia, TypeName);\
            jia = __starter_move(yi, TypeName);\
            yi = __starter_move(bin, TypeName);
...
/**< some extra work is done to fit bid-rational iterator */
    ...
    insertionSort(BidIter left, BidIter right) {
        ...
        for (BidIter cur = left; cur != right; ++cur) {
            ...
            ++adv;
            if (*adv < *cur) {
                ValType tmp = __starter_move(*adv, ValType);
                *adv = __starter_move(*cur, ValType);
                BidIter last(cur);
                BidIter last_adv(cur);
                while (tmp < *last && last_adv != left) {
                    last_adv = last;
                    --last;
                    *last_adv = __starter_move(*last, ValType);
                }
                *last_adv = __starter_move(tmp, ValType);
            }
        }
    }
...
//----------------------------------------------------------------------------
    ...
    merge (BidIter left1, BidIter right1,
        ...
        while (left1 != right1 && left2 != right2) {
            if (*left1 < *left2) {
                *dest = __starter_move(*left1, ValType);
                ++left1;
            }
            else {
                *dest = __starter_move(*left2, ValType);
                ++left2;
            }
            ++dest;
        }
        ...
        ValType* start = arr;
        while (start != dest) {
            *destination = __starter_move(*start, ValType);
            ++start;
            ++destination;
        }
        delete [] arr;
