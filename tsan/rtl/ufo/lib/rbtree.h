...
/**
 * this rb-tree is the same as std::map, not for direct use, see TreeMap.h (Bowen 2017-10-13)
 *
 */
...
//                    "custom node must be derived from \"Starter::BaseNode\"\n");
//
//
...
//                    >::value,
//        " custom node must define type \"KeyType\" and define function \"getKey\"\n");
...
public:
  //  read-only
  void foreach(std::function<void(const Node *)> visit) const { S_forward_iterate(head_.parent_, visit); }

  void foreach(std::function<void(Node *)> visit) { S_forward_iterate(head_.parent_, visit); }
...
public:
  ...
  bool empty() const noexcept { return 0 == this->size_; }
...
//-----------------------------------------------------------------------------
public:
  iterator root() noexcept { return iterator(head_.parent_); }

  const_iterator root() const noexcept { return const_iterator(head_.parent_); }

  iterator begin() noexcept { return iterator(head_.left_); }

  iterator end() noexcept { return iterator(addressOf(this->head_)); }

  const_iterator cbegin() const noexcept { return const_iterator(head_.left_); }

  const_iterator cend() const noexcept { return const_iterator(addressOf(head_)); }

  iterator max() noexcept { return iterator(head_.right_); }

  const_iterator max() const noexcept { return const_iterator(head_.right_); }
...
//-----------------------------------------------------------------------------
  ...
  reference front() noexcept { return static_cast<Node *>(head_.left_)->value; }

  reference back() noexcept { return static_cast<Node *>(head_.right_)->value; }

  const_reference front() const noexcept { return static_cast<Node *>(head_.left_)->value; }

  const_reference back() const noexcept { return static_cast<Node *>(head_.right_)->value; }
  ...
  BaseNode *data() noexcept { return head_.parent_; }

  const BaseNode *data() const noexcept { return head_.parent_; }
