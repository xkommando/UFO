...
TSAN_INTERCEPTOR(void*, __libc_memalign, uptr align, uptr sz) {
  ...
  return user_alloc(thr, caller_pc, sz, align);
}
...
TSAN_INTERCEPTOR(char*, strcpy, char *dst, const char *src) {  // NOLINT
  ...
  MemoryAccessRange(thr, caller_pc, (uptr)dst, srclen + 1, true);
  MemoryAccessRange(thr, caller_pc, (uptr)src, srclen + 1, false);
...
TSAN_INTERCEPTOR(char*, strncpy, char *dst, char *src, uptr n) {
  ...
  MemoryAccessRange(thr, caller_pc, (uptr)dst, n, true);
  MemoryAccessRange(thr, caller_pc, (uptr)src, min(srclen + 1, n), false);
...
TSAN_INTERCEPTOR(void *, mmap, void *addr, SIZE_T sz, int prot, int flags,
  ...
  if (res != MAP_FAILED) {
    ...
    if (thr->ignore_reads_and_writes == 0)
      MemoryRangeImitateWrite(thr, caller_pc, (uptr)res, sz);
    else
      MemoryResetRange(thr, caller_pc, (uptr)res, sz);
  }
  return res;
}
...
TSAN_INTERCEPTOR(void*, valloc, uptr sz) {
  ...
  return user_alloc(thr, caller_pc, sz, GetPageSizeCached());
}
#endif
...
TSAN_INTERCEPTOR(int, posix_memalign, void **memptr, uptr align, uptr sz) {
  ...
  *memptr = user_alloc(thr, caller_pc, sz, align);
  return 0;
}
#endif
...
TSAN_INTERCEPTOR(int, pthread_create,
  ...
  if (res == 0) {
    int tid = ThreadCreate(thr, caller_pc, *(uptr*)th,
...
static int cond_wait(ThreadState *thr, uptr pc, ScopedInterceptor *si,
  ...
  bw::ufo::on_cond_wait(thr, pc, (u64)c, (u64)m);
...
INTERCEPTOR(int, pthread_cond_wait, void *c, void *m) {
  ...
  return cond_wait(thr, caller_pc, &si, (int (*)(void *c, void *m, void *abstime))REAL(
                                     pthread_cond_wait),
                   cond, m, 0);
}
...
INTERCEPTOR(int, pthread_cond_timedwait, void *c, void *m, void *abstime) {
  ...
  return cond_wait(thr, caller_pc, &si, REAL(pthread_cond_timedwait), cond, m,
                   abstime);
}
...
INTERCEPTOR(int, pthread_cond_signal, void *c) {
  ...
  bw::ufo::on_cond_signal(thr, caller_pc, (u64)c);
...
INTERCEPTOR(int, pthread_cond_broadcast, void *c) {
  ...
  bw::ufo::on_cond_broadcast(thr, caller_pc, (u64)c);
...
TSAN_INTERCEPTOR(int, pthread_mutex_trylock, void *m) {
  ...
  if (res == 0 || res == EOWNERDEAD)
    MutexLock(thr, caller_pc, (uptr)m, /*rec=*/1, /*try_lock=*/true);
  return res;
}
...
TSAN_INTERCEPTOR(int, pthread_mutex_timedlock, void *m, void *abstime) {
  ...
  if (res == 0) {
    MutexLock(thr, caller_pc, (uptr)m);
  }
  return res;
}
#endif
...
TSAN_INTERCEPTOR(int, pthread_spin_init, void *m, int pshared) {
  ...
  if (res == 0) {
    MutexCreate(thr, caller_pc, (uptr)m, false, false, false);
  }
  return res;
}
...
TSAN_INTERCEPTOR(int, pthread_rwlock_init, void *m, void *a) {
  ...
  if (res == 0) {
    MutexCreate(thr, caller_pc, (uptr)m, true, false, false);
  }
  return res;
}
...
TSAN_INTERCEPTOR(int, pthread_rwlock_rdlock, void *m) {
  ...
  if (res == 0) {
    MutexReadLock(thr, caller_pc, (uptr)m);
  }
  return res;
}
...
TSAN_INTERCEPTOR(int, pthread_rwlock_tryrdlock, void *m) {
  ...
  if (res == 0) {
    MutexReadLock(thr, caller_pc, (uptr)m, /*try_lock=*/true);
  }
  return res;
}
...
TSAN_INTERCEPTOR(int, open, const char *name, int flags, int mode) {
  ...
  READ_STRING(thr, caller_pc, name, 0);
...
TSAN_INTERCEPTOR(uptr, fread, void *ptr, uptr size, uptr nmemb, void *f) {
  // libc file streams can call user-supplied functions, see fopencookie.
  {
    ...
    MemoryAccessRange(thr, caller_pc, (uptr)ptr, size * nmemb, true);
  }
...
TSAN_INTERCEPTOR(uptr, fwrite, const void *p, uptr size, uptr nmemb, void *f) {
  // libc file streams can call user-supplied functions, see fopencookie.
  {
    ...
    MemoryAccessRange(thr, caller_pc, (uptr)p, size * nmemb, false);
  }
...
TSAN_INTERCEPTOR(int, puts, const char *s) {
  ...
  MemoryAccessRange(thr, caller_pc, (uptr)s, internal_strlen(s), false);
  return REAL(puts)(s);
}
...
#define COMMON_INTERCEPTOR_WRITE_RANGE(ctx, ptr, size)                    \
  MemoryAccessRange(((TsanInterceptorContext *)ctx)->thr,                 \
                    ((TsanInterceptorContext *)ctx)->caller_pc, (uptr)ptr, size, \
                    true)
...
#define COMMON_INTERCEPTOR_MUTEX_LOCK(ctx, m) \
  MutexLock(((TsanInterceptorContext *)ctx)->thr, \
            ((TsanInterceptorContext *)ctx)->caller_pc, (uptr)m)
