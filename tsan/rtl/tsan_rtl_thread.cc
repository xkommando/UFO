...
void ThreadContext::OnCreated(void *arg) {
  ...
  bw::ufo::on_thread_created(args->thr->tid, this->tid, args->pc);
...
//  Printf("\r\n OnStarted args->thr->tid %d, cur_thread()->tid %d   %d\r\n", args->thr->tid, cur_thread()->tid, cur_thread() == thr);
  bw::ufo::on_thread_start(thr, args->stk_addr, args->stk_size, args->tls_addr, args->tls_size);
...
void MemoryAccessRange(ThreadState *thr, uptr pc, uptr addr,
  ...
  if (size == 0)
    return;
  bw::ufo::on_mem_range_acc(thr, pc, addr, size, is_write);
//
